 ...
 * @details Identical copy of os/hal/ports/TI/AM67/am67_padcfg.h. It has no
 *          classic-HAL dependency -- it is silicon-level register access
 *          only -- but the classic and XHAL AM67 directories cannot share
 *          an include path without their differently-shaped hal_*_lld.h
 *          files colliding, so this stays a duplicate rather than a shared
 *          include until the two trees have a common location to live in.
 *          Keep both copies identical if this file changes.
